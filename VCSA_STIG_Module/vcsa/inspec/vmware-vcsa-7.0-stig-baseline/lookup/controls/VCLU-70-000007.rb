control 'VCLU-70-000007' do
  ...
  desc  'check', "
    ...
    # find /var/log/vmware/lookupsvc -xdev -type f -a '(' -not -user root -a -not -user lookupsvc -o -not -group root -a -not -group lookupsvc ')' -exec ls -ld {} \;
  ...
  command("find '#{input('logPath')}' -type f -xdev").stdout.split.each do |fname|
    describe file(fname) do
      ...
      its('owner') { should cmp('root').or cmp('lookupsvc') }
      its('group') { should cmp('root').or cmp('lookupsvc') }
    end
  end
end
